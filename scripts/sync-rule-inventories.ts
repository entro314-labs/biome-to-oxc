 ...
 *   - node_modules/@biomejs/biome/configuration_schema.json  -> docs/biome-rules.tsv
 *                                                            -> docs/biome-formatter-options.tsv
 *
 * Usage:
...
async function readTsgolintStatuses(): Promise<Map<string, 'implemented' | 'not-implemented'>> {
  ...
  for (const match of readme.matchAll(
    ...
    statuses.set(match[2] as string, match[1] === 'x' ? 'implemented' : 'not-implemented')
  }
...
async function buildOxlintRulesTsv(oxlintRuleNames: string[]): Promise<string> {
  return toTsv(null, oxlintRuleNames.map(splitOxlintRuleName).sort(compareRows))
}

async function buildTsgolintRulesTsv(
...
async function buildOxlintVsTsgolintTsv(
...
/** Group-level switches that sit alongside the rules and are not rules themselves. */
const BIOME_GROUP_LEVEL_KEYS = new Set(['recommended', 'all', 'preset'])

const BIOME_FORMATTER_SURFACES = {
  FormatterConfiguration: 'formatter',
  CssFormatterConfiguration: 'css.formatter',
  GraphqlFormatterConfiguration: 'graphql.formatter',
  GritFormatterConfiguration: 'grit.formatter',
  HtmlFormatterConfiguration: 'html.formatter',
  JsFormatterConfiguration: 'javascript.formatter',
  JsonFormatterConfiguration: 'json.formatter',
  OverrideFormatterConfiguration: 'overrides[].formatter',
} as const

async function readBiomeDefinitions(): Promise<Record<string, unknown>> {
  const schema = await readJson(join(modulesDir, '@biomejs', 'biome', 'configuration_schema.json'))
  return requireRecord(schema.$defs ?? schema.definitions, 'Biome schema definitions')
}

async function buildBiomeRulesTsv(definitions: Record<string, unknown>): Promise<string> {
  const rows: string[][] = []

  for (const group of BIOME_RULE_GROUPS) {
    for (const rule of propertyNames(definitions[group], `Biome ${group}`)) {
      if (!BIOME_GROUP_LEVEL_KEYS.has(rule)) {
        rows.push([rule, group.toLowerCase()])
      }
    }
  }

  if (rows.length < 300) {
    throw new Error(`Read only ${rows.length} Biome rules; the schema layout likely changed.`)
  }

  return toTsv(['rule', 'group'], rows.sort(compareRows))
}

async function buildBiomeFormatterOptionsTsv(
  definitions: Record<string, unknown>,
): Promise<string> {
  const rows: string[][] = []

  for (const [definitionName, surface] of Object.entries(BIOME_FORMATTER_SURFACES)) {
    for (const option of propertyNames(definitions[definitionName], `Biome ${definitionName}`)) {
      rows.push([option, surface])
    }
  }
...
async function main(): Promise<void> {
  ...
  const biomeDefinitions = await readBiomeDefinitions()

  const inventories: Array<[fileName: string, content: string]> = [
    ['oxlint-rules.tsv', await buildOxlintRulesTsv(oxlintRuleNames)],
    ['oxfmt-rules.tsv', await buildOxfmtOptionsTsv()],
    ['tsgolint-rules.tsv', await buildTsgolintRulesTsv(tsgolintStatuses)],
    ['oxlint-vs-tsgolint.tsv', await buildOxlintVsTsgolintTsv(oxlintRuleNames, tsgolintStatuses)],
    ['biome-rules.tsv', await buildBiomeRulesTsv(biomeDefinitions)],
    ['biome-formatter-options.tsv', await buildBiomeFormatterOptionsTsv(biomeDefinitions)],
  ]
