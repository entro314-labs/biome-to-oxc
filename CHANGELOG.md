...
- Biome's `graphql` section is migrated instead of being reported as having no Oxc equivalent. Oxfmt has formatted GraphQL natively since 0.57, so `graphql.formatter.enabled: false` now excludes `**/*.{graphql,gql}` from Oxfmt, and `indentStyle`, `indentWidth`, `lineWidth`, `lineEnding`, `bracketSpacing` and `trailingNewline` map onto an Oxfmt override for those files, top-level and per Biome override. `quoteStyle` is accepted without a loss, since GraphQL strings are always double-quoted and neither tool acts on it. A conformance test runs both binaries over the same GraphQL source and checks the output matches. The GraphQL migration hint no longer suggests `graphql-prettier` or says Oxfmt may support GraphQL later.
- Mappings for two Biome rules whose partial Oxlint counterparts already existed, both reported as semantic losses: `noInlineStyles` → `react/forbid-dom-props` with `style` forbidden, which misses `style` passed through `React.createElement()` and in HTML, Vue, Svelte or Astro templates; and `noUnresolvedImports` → `import/named`, which reports missing named exports in JavaScript files only and does not report unresolvable import paths.

### Changed

- Toolchain refreshed to Oxlint 1.85.0, Oxfmt 0.70.0, oxlint-tsgolint 7.0.2003 and Biome 2.5.14, and the inventories under `docs/` regenerated from their schemas. Oxlint 1.82–1.85 and Oxfmt 0.67–0.70 shipped no new rules and no new Oxfmt options, so `docs/oxlint-rules.tsv`, `docs/oxfmt-rules.tsv` and `docs/oxlint-vs-tsgolint.tsv` are unchanged; the one new rule option, `no-unmodified-loop-condition`'s `checkConditionalExpressions`, has no Biome counterpart to migrate from. oxlint-tsgolint 7.0.2003 implements `no-generated-empty-object-type`, which `docs/tsgolint-rules.tsv` now lists, but Oxlint 1.85 rejects `typescript/no-generated-empty-object-type` as an unknown rule, so it cannot be enabled until an Oxlint release exposes it; Biome has no equivalent rule either way. Biome 2.5.12–2.5.14 added 22 nursery rules; the seven with an Oxlint counterpart are mapped (see Added), and `noBunModules`, `noInvalidFileInputAccept`, `noJsonUnsafeValues`, `noObsoleteTags`, `noSvelteAtDebugTags`, `noSvelteAtHtmlTags`, `noThisOutsideOfClass`, `noVueDeprecatedScopedSlots`, `noVueUndeclaredDirectives`, `noXorAsExponentiation`, `useBetterDomTraversing`, `useConsistentObjectKeys`, `useFlatMathMinMax`, `useLayeredStyles` and `useReactNamingConvention` stay unmapped because Oxlint has no rule for them.

## [3.5.0] - 2026-09-03

### Added

- `useArrayFind`, `useIncludes` and `useStringStartsEndsWith` now migrate to `unicorn/prefer-array-find`, `unicorn/prefer-includes` and `unicorn/prefer-string-starts-ends-with` when the migration does not turn on type-aware linting. Their only Oxlint counterpart was a tsgolint rule that never runs without `--type-aware`, so the default migration silently dropped those diagnostics. The fallback replaces the type-aware rule instead of joining it, so enabling the type-aware profile later never double-reports a line. `unicorn/prefer-string-starts-ends-with` covers only the anchored-regex form of what Biome flags, so the remainder is reported as a semantic loss. Overrides use the same substitution as the top-level config. ([7e03794](https://github.com/entro314-labs/biome-to-oxc/commit/7e03794))
