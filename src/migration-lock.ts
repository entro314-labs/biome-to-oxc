...
const LOCK_FILE_NAME = '.biome-to-oxc.lock'
...
/**
 * Takes an exclusive, project-scoped lock for the duration of a migration.
 *
 * Snapshot, write and restore are not atomic across processes: two concurrent migrations
 * can interleave their writes, and a rollback in one can restore files the other just
 * wrote. The lock serializes them.
 */
export async function acquireMigrationLock(
  ...
  if (existing && !isStale(existing, now) && isProcessAlive(existing.pid)) {
    throw new MigrationLockedError(
      `Another biome-to-oxc migration is running in ${projectDir} (pid ${existing.pid}). Wait for it to finish, or remove ${lockPath} if that process is gone.`,
    )
  }

  if (existing) {
    reporter.warn(`Reclaiming stale migration lock left by pid ${existing.pid} at ${lockPath}.`)
  }
  ...
  try {
    ...
    if (existing) {
      ...
      throw new MigrationLockedError(
        `Could not acquire the migration lock at ${lockPath}: ${err instanceof Error ? err.message : String(err)}`,
      )
    }
  }
